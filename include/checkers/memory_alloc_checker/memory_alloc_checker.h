#ifndef _memory_alloc_checker_H
#define _memory_alloc_checker_H
...
class memory_alloc_checker : public BasicChecker {
private:
...
//	}
	memory_alloc_checker(ASTResource *resource, ASTManager *manager,
                  CallGraph *call_graph, Config *configure)
      : BasicChecker(resource, manager, call_graph, configure){

        std::unordered_map<std::string, std::string> ptrConfig =configure->getOptionBlock("memory_alloc_checker");
	}
